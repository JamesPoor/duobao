...
def find(jd, one):
    if jd:
        ...
        while json_data['code'] == 0:
            ...
            cursor = con.cursor()
            cursor.execute('update fa_anchor set status=2, rank=' + str(json_data['data']) + ' where id=' + str(one[0]))
...
con = mysql.connector.connect(**config)
cursor = con.cursor()
cursor.execute('select * from fa_anchor where status = 0 or status = 3')
